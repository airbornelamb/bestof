# bestof
Personal list of awesome things

- master [awesome list](https://github.com/sindresorhus/awesome)

## Linux

- list of [awesome sysadmin](https://github.com/kahun/awesome-sysadmin)
- devops wiki [devopswiki](https://github.com/Leo-G/DevopsWiki)
- AMAZING free server monitor [netdata](https://github.com/firehol/netdata)
- log viewer [log.io](https://github.com/NarrativeScience/Log.io)
- server dashboard [linux-dash](https://github.com/afaqurk/linux-dash)
- server [cockpit](https://github.com/cockpit-project/cockpit)
- better webmin/usermin theme [authentic-theme](https://github.com/authentic-theme/authentic-theme)
- dynamic dns [dynu](https://www.dynu.com/)
- network monitor [PRTG](https://www.paessler.com/prtg)
- list of [awesome shell](https://github.com/alebcay/awesome-shell)

- Server hardening
  - security auditing tool [lynis](https://github.com/CISOfy/lynis)
  - ansible role [security hardening](https://github.com/openstack/ansible-hardening)
  - security [script](https://github.com/MattDelac/set_up_security_server)
  - [logwatch analyzer](https://www.digitalocean.com/community/tutorials/how-to-install-and-use-logwatch-log-analyzer-and-reporter-on-a-vps)
  - [initial server setup](https://www.digitalocean.com/community/tutorials/initial-server-setup-with-ubuntu-18-04)
  - [7 steps to secure servers](https://www.digitalocean.com/community/tutorials/7-security-measures-to-protect-your-servers)
  - [additional recommended steps](https://www.digitalocean.com/community/tutorials/additional-recommended-steps-for-new-ubuntu-14-04-servers)
  - [secure vpn server](https://blog.hsp.dk/how-to-setup-vpn-server-on-raspberry-pi-or-digitalocean/)
  - [my first 5 mins on a server](https://plusbryan.com/my-first-5-minutes-on-a-server-or-essential-security-for-linux-servers)
  - [first 10 mins on a new server](https://www.codelitt.com/blog/my-first-10-minutes-on-a-server-primer-for-securing-ubuntu/)
  - [securing linux web server](https://hackernoon.com/securing-your-linux-web-server-2be683c223eb)

- Utilities
 - aria2 ui [webui-aria2](https://github.com/ziahamza/webui-aria2)
 - search and save shell snippets [borg](https://github.com/ok-borg/borg)
 - misc [shell scripts](https://github.com/lahwaacz/Scripts)
 
- Backup
  - RSYNC.NET is AWESOME!
  - borg yaml script [borgmatic](https://github.com/witten/borgmatic)
  - s3 filesystem [s3fs](https://github.com/s3fs-fuse/s3fs-fuse)

- Docker
  - docker [curriculum](https://github.com/prakhar1989/docker-curriculum)
  - management UI [portainer](https://github.com/portainer/portainer)
  - security check [docker-bench](https://github.com/docker/docker-bench-security)
  - mailserver [docker-mailserver](https://github.com/tomav/docker-mailserver)
  - mail selivery platform [postal](https://github.com/atech/postal)

## Raspberry Pi

- [awesome raspberry pi](https://github.com/thibmaek/awesome-raspberry-pi)
- ad-blocker [PiHole](https://github.com/pi-hole/pi-hole)
- VPN Server [PiVPN](https://github.com/pivpn/pivpn)
- Enforcing Safesearch [safesearch](https://github.com/Kiwifruta/safesearch)
- lan print and pdf server [ultimate pi](https://extremeshok.com/6138/ultimate-raspberry-pi-lan-print-scan-and-pdf-server-cups-airprint-ipp-cups-pdf-sanescan-sanetwain-phpsane-scanner-webinterface-samba-remote-file-access/)
- [pibakery](https://github.com/davidferguson/pibakery)

## Online Tools

- Misc
  - Diff comparison [Mergely](https://github.com/wickedest/Mergely)
  
## Markdown/Text Processing

- sensible [vim defaults](https://github.com/tpope/vim-sensible)
- learn latex [begin latex](https://github.com/luongvo209/Begin-Latex-in-minutes)
- rst [cheat-sheet](https://github.com/ralsina/rst-cheatsheet)
- text processing [cli text processing](https://github.com/learnbyexample/Command-line-text-processing)
- diagrams [mermaid](https://github.com/knsv/mermaid)
- mindmaps [text2mindmap](https://github.com/tobloef/text2mindmap)
- csv files [csvkit](https://github.com/wireservice/csvkit)
- tufte style [tufte-pandoc-css](https://github.com/jez/tufte-pandoc-css)
- writing letters in markdown [pandoc-letter](https://github.com/aaronwolen/pandoc-letter)
- better bibtexfor zotero [zotero-better-bibtex](https://github.com/retorquere/zotero-better-bibtex)
- [emacs starter kit](https://github.com/technomancy/emacs-starter-kit)
- another [emacs starter](https://github.com/kjhealy/emacs-starter-kit)

- doc generator [docsify](https://github.com/QingWei-Li/docsify)
- best theme for mkdocs [mkdocs-material](https://github.com/squidfunk/mkdocs-material)
- documentation toolchain [gitbook](https://github.com/GitbookIO/gitbook)
- sphinx [awesome sphinx](https://github.com/yoloseem/awesome-sphinxdoc)
- [docusaurus](https://github.com/facebook/Docusaurus)
- [pelican site generator](https://github.com/getpelican/pelican)

## Business

MSP Tools

Connectwise, Labtech, Auvik, Quosal, IT Glue, Brightgauge, Network Detective = MSP in a box

[zamad helpdesk](https://github.com/zammad/zammad)
[faveo helpdesk](https://github.com/ladybirdweb/faveo-helpdesk)
[helpy](https://github.com/helpyio/helpy)

[lavagna issue tracking](https://github.com/digitalfondue/lavagna)


Piwik - for pictures and photo gallery;

Seafile Pro - for syncing and file hosting (far far better than nextcloud)

Plex - for music, movies, tv series

SOGo - as a cardav and caldav server

ISP Mail (google it) - as a personal mail server for all my domains;

VTiger - for CRM

FreePBX - for all my SIP accounts and as an XMPP server

Firefly - for personal accounting and finance

Invoice Plane - for issuing invoices and finance [here](https://github.com/InvoicePlane/InvoicePlane)

[invoice ninja](https://github.com/invoiceninja/invoiceninja)

Mautic - for publicity and marketing campaigns

Calibre server - for ebooks and magazines

Wallabag - for saving news articles and webpages around the web

TT-RSS - as an rss server

Turtl - for private and encrypted notes

Nethserver - as a ldap and centralized business server

Pi-Hole - as a DNS server

Server Status (a re-write of Mojeda's script) - to monitor my servers/VPS

OpenVPN - as VPN server

Nextcloud - just because I may one day figure out some use for it.

Webmin - in some servers, to make it easier to administrate

Wordpress - for my blogs and websites

## Windows

- Productivity
  - keyboard macros [autohotkey](https://autohotkey.com/)
  - macro creator [pulover](https://github.com/Pulover/PuloversMacroCreator)
  - text snippets [lintalist](https://github.com/lintalist/lintalist)
  - todo.txt
- Cleanup
  - Good [Ccleaner](https://www.ccleaner.com)
- Antivirus/malware
  - Best Paid AV [ESET](https://www.eset.com)
  - Best Free AV [Avast](https://www.avast.com)
  - Best Free anti-malware [Malwarebytes](https://www.malwarebytes.com/)
  - Removing stupid mcafee [MCPR](http://us.mcafee.com/apps/supporttools/mcpr/mcpr.asp)
- Passwords
  - KeepassX
  - Enpass
- File synchronization
  - [syncthing](https://github.com/syncthing/syncthing)
  - [sparkleshare](https://github.com/hbons/SparkleShare)
- Package manager
  - Pretty good [Ninite](https://ninite.com)
  - Extensive [Chocolatey](https://chocolatey.org)
  - Up and comer, but spotty [Just-install](https://just-install.it/)
- CLI
  - Awesome cmder (includes git) [Cmder](https://github.com/cmderdev/cmder)
- Utilities
  - Decent network scanner [Softperfect network scanner](https://www.softperfect.com/products/networkscanner/)
  - remote connection manager [mremoteng](https://github.com/mRemoteNG/mRemoteNG)
  - System information tool [Speccy](https://www.ccleaner.com/speccy)
  - More windows utilities [windows tools](https://github.com/dloss/windows-tools-on-cdrom)
  - Good remote vnc (5 devices home free) [realvnc](https://www.realvnc.com/)
  - Good remote vnc  [teamviewer](https://www.teamviewer.us/latest-version/)

  
## Voip

- Sip Server
  - Good [3CX](https://www.3cx.com/)
  
## Coding

- [free programming books](https://github.com/EbookFoundation/free-programming-books)
- [hackathon-starter](https://github.com/sahat/hackathon-starter)
- [development environment](https://github.com/jondot/awesome-devenv)
- [every programmer should know](https://github.com/mtdvio/every-programmer-should-know)
- [webdev best practices](https://github.com/google/WebFundamentals)
- [programming fonts](https://github.com/braver/programmingfonts)
- [awesome cheatsheets](https://github.com/LeCoupa/awesome-cheatsheets)
- [learning tools](https://github.com/scottslowe/learning-tools)
- [frontend-dev-bookmarks](https://github.com/dypsilon/frontend-dev-bookmarks)
- [learn regex](https://github.com/zeeshanu/learn-regex)
- [how to use git](https://github.com/k88hudson/git-flight-rules)
- [learnyoubash](https://github.com/denysdovhan/learnyoubash)
- [bash-handbook](https://github.com/denysdovhan/bash-handbook)
- [ansible tutorial](https://github.com/leucos/ansible-tuto)
- [4 minute web design](https://jgthms.com/web-design-in-4-minutes/)
- [free html/css tutorial](http://marksheet.io/)
- [front-end guide](https://github.com/grab/front-end-guide)
- [front-end checklist](https://github.com/thedaviddias/Front-End-Checklist)
- [html-reference](https://github.com/jgthms/html-reference)
- [css-reference](https://github.com/jgthms/css-reference)
- [js cheatsheet](https://github.com/mbeaudru/modern-js-cheatsheet)
- [css pro tips](https://github.com/AllThingsSmitty/css-protips)
- [you don't know js](https://github.com/getify/You-Dont-Know-JS)
- [you don't need js](https://github.com/you-dont-need/You-Dont-Need-JavaScript)
- [javascript learning resources](https://github.com/micromata/awesome-javascript-learning)
- [html5 boilerplate](https://github.com/h5bp/html5-boilerplate)
- [skeleton](https://github.com/dhg/Skeleton)
- [stock photography etc](https://github.com/neutraltone/awesome-stock-resources)
- [insomnia rest and graphql client](https://github.com/getinsomnia/insomnia)
- [coding practice](https://github.com/exercism/exercism.io)
